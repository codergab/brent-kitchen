<template>
  <div v-if="profile">
    <div class="img1">
      <img :src="imagery(profile.avatar)" class="rounded" />
    </div>
    <div class="card shadow-lg2 columns">
      ...
      <h1>Enter Address:</h1>
      <div class="margin">
        <textarea placeholder="Enter Address Here*" v-model="profile.address"></textarea>
      </div>
    </div>
    <div class="footer">
...
export default {
  ...
  async asyncData({ store }) {
    let profile = {};
    let userDetails = await store.state.auth.user;
  ...
  methods: {
    ...
    async placeOrder() {
      ...
      // }
      this.payWithPaystack({ ...this.profile, amount: this.getTotal });
    },
    payWithPaystack(payload) {
      const app = this;
      var handler = PaystackPop.setup({
        key: "pk_test_a8f27f62e1188fab5e660d9af95f4a8b5b5ab494",
        email: payload.email,
        amount: `${payload.amount}00`,
        currency: "NGN",
...
.disable {
  color: #f6f7fa;
  background: linear-gradient(87deg, #000 0, #555 100%) !important;
  border-color: #fb6340;
}
.email {
  letter-spacing: 1px;
